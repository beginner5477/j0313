...
public class A4 {
	public static void main(String[] args) throws ParseException {
		...
		System.out.print("날짜를 입력해주세요(yyyy-MM-dd형식으로 입력바람):");
		String ansDate = sc.nextLine();
		String[] arr1 = ansDate.split("-");
		int n4 = Integer.parseInt(arr1[0]);
		int n5 = Integer.parseInt(arr1[1]);
		int n6 = Integer.parseInt(arr1[2]);
		LocalDateTime startDateTime = LocalDateTime.of(n4, n5, n6, 0, 0, 0);
		...
		for(int i = 0; i < dateData.length; i++)
		{
			...
			if(compareDateTime.until(startDateTime, ChronoUnit.DAYS) < 0) {}
			else if(compareDateTime.until(startDateTime, ChronoUnit.DAYS) >= 365)
			{
		...
		for(int i = 0; i < dateData.length; i++)
		{
			...
			LocalDateTime compareDateTime = LocalDateTime.of(n1, n2, n3, 0, 0, 0);
			if(compareDateTime.until(startDateTime, ChronoUnit.DAYS) < 0) {}
			else if(compareDateTime.until(startDateTime, ChronoUnit.DAYS) < 30)
			{
				System.out.print(dateData[i][0]+" ");
		...
		for(int i = 0; i < dateData.length; i++)
		{
			...
			if(compareDateTime.until(startDateTime, ChronoUnit.DAYS) < 0) {}
			else if(compareDateTime.until(startDateTime, ChronoUnit.DAYS) < 1)
			{
				System.out.print(dateData[i][0]+" ");
